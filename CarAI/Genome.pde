class Genome {
  ...
  int nextNode = 0;
  int biasNode;
  boolean networkGenerated = false;
  ...
  //-------------------------------------------------------------------------------------------------------------------------------
  //mutates the genome
  void mutate(ArrayList<connectionHistory> innovationHistory, float wChance, float cChance, float nChance) {
    if (genes.size() ==0) {
      addConnection(innovationHistory);
    }

    float rand1 = random(1);
    if (rand1<wChance) { // 80% of the time mutate weights
