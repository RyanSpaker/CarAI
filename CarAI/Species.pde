class Species {
  ...
  Player giveMeBaby(ArrayList<connectionHistory> innovationHistory, float wChance, float cChance, float nChance, float mChance) {
    Player baby;
    ...
    if (rand3<mChance) {
      baby.brain.mutate(innovationHistory, wChance, cChance, nChance);//mutate that baby brain
    }
    return baby;
  }
